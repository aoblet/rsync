...
BOOL io_start_buffering_out(int f_out)
{
	...
	alloc_xbuf(&iobuf.out, ROUND_UP_1024((bufferio_size ? bufferio_size : IO_BUFFER_SIZE) * 2));
	iobuf.out_fd = f_out;
...
BOOL io_start_buffering_in(int f_in)
{
	...
	alloc_xbuf(&iobuf.in, ROUND_UP_1024(bufferio_size ? bufferio_size : IO_BUFFER_SIZE));
	iobuf.in_fd = f_in;
...
void io_start_multiplex_out(int fd)
{
	...
	if (!iobuf.msg.buf)
		alloc_xbuf(&iobuf.msg, ROUND_UP_1024(bufferio_size ? bufferio_size : IO_BUFFER_SIZE));
